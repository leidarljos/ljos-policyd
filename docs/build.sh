#!/usr/bin/env bash
# Build the ljos-policyd Sphinx/Shibuya site: org → rst → html
# Usage (from repo root or this directory):
#   ./docs/build.sh
# Optional: LJOS_POLICYD_DOC_EXPORTER=emacs|pandoc  LJOS_POLICYD_DOC_VENV=path
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"

VENV="${LJOS_POLICYD_DOC_VENV:-$ROOT/.venv-docs}"
